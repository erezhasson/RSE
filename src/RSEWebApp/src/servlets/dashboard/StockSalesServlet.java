...
public class StockSalesServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int commandApplied = user.getAppliedCommands().size();
            int completedDeals = user.getCompletedCommands().size();
            float stockSales = commandApplied != 0 ? (float)(completedDeals / commandApplied) * 100 : 0;

            out.println(stockSales);
            out.flush();
