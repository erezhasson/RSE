    ...
    protected enum CommandType {
        LMT{
            public boolean MatchingByCommandType(RizpaCommand originalCommand, RizpaCommand comparedCommand){
                ...
                return matching && differentUsersCommands(originalCommand, comparedCommand);
            }
        },
        MKT {
            public boolean MatchingByCommandType(RizpaCommand originalCommand, RizpaCommand comparedCommand){
                originalCommand.price = comparedCommand.price;
                return differentUsersCommands(originalCommand, comparedCommand);
            }
        },
        FOK {
            public boolean MatchingByCommandType(RizpaCommand originalCommand, RizpaCommand comparedCommand){
                return LMT.MatchingByCommandType(originalCommand, comparedCommand);
            }
        },
        IOC {
            public boolean MatchingByCommandType(RizpaCommand originalCommand, RizpaCommand comparedCommand){
        ...
        public boolean differentUsersCommands(RizpaCommand originalCommand, RizpaCommand comparedCommand) {
            boolean isBuyingCommand = originalCommand instanceof BuyingCommand;
            User seller = isBuyingCommand ? comparedCommand.seller : originalCommand.seller;
            User buyer = isBuyingCommand ? originalCommand.buyer : comparedCommand.buyer;

            return !seller.getName().equals(buyer.getName());
        }
