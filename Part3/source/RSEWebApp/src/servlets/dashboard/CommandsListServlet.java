...
public class CommandsListServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (ServletUtils.isAdmin(user)) {
                out.println(engine.getAllPendingCommands().size());
            }
            else {
                out.println(user.getAppliedCommands().size());
            }

            out.flush();
        } catch (UserException | StockException ignored) {
        }
    }
